 ...
 *
 */
class QuestionDuaFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        isDataQuestion = activity?.let { PrefSaveQuestionDua(it).getData().toString() }.toString()
        Log.e("data ", "data question dua $isDataQuestion")
    ...
    private fun initData() {
        var data: List<ModelSatuanKerja> =
            listOf(
                ModelSatuanKerja(
                    "1",
                    "Permintaan/permohonan data"
                ),
                ModelSatuanKerja(
                    "2",
                    "Kunjungan kerja/sekolah/kampus"
                ),
                ModelSatuanKerja(
                    "3",
                    "PKL/Magang"
                ...
                ModelSatuanKerja(
                    "4",
                    "Workshop/seminar/training/bimtek"
                ),
                ModelSatuanKerja("5", "Pameran"),
                ModelSatuanKerja(
                    "6",
                    "Lainya"
                )
        ...
        adapter.setOnItemClickListener(object : JabawanDuaAdapter.OnItemClickListener {
            override fun onItemClick(position: Int) {
                activity?.let { PrefSaveQuestionDua(it).setData(data.get(position).id) }
                openFragment(QuestionTigaFragment())
