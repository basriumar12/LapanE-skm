package com.example.lapane_skm.ui.question.fragment_lima
 ...
 *
 */
class QuestionLimaFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        tvNo?.text ="5"
        tvQuestion?.text ="Pendidikan *"
        initData()


        val isDataQuestion = activity?.let { PrefSaveQuestionLima(it).getData().toString() }.toString()
        Log.e("data ","data question lima $isDataQuestion")
        ...
        tvNext?.setOnClickListener {
            openFragment(Question6Fragment())
        }
    ...
    private fun initData() {
        ...
        val adapter = JabawanLimaAdapter(activity,data)
        rvDataQuestion?.adapter = adapter
        adapter.notifyDataSetChanged()

        adapter.setOnItemClickListener(object : JabawanLimaAdapter.OnItemClickListener {
            override fun onItemClick(position: Int) {
                activity?.let { PrefSaveQuestionLima(it).setData(data.get(position).id) }
                openFragment(Question6Fragment())
