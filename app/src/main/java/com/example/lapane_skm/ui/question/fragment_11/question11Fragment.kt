package com.example.lapane_skm.ui.question.fragment_11
 ...
 *
 */
class Question11Fragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        isDataQuestion = activity?.let { PrefSaveQuestion11(it).getData().toString() }.toString()
        Log.e("data ", "data question 11 $isDataQuestion")
    ...
    private fun initData() {
        tvNo?.text ="11"
        tvQuestion?.text ="Kompetensi/kemampuan petugas dalam pelayanan? *"
        ...
        val adapter = Jabawan11Adapter(activity, data)
        rvDataQuestion?.adapter = adapter
        adapter.notifyDataSetChanged()

        adapter.setOnItemClickListener(object : Jabawan11Adapter.OnItemClickListener {
            override fun onItemClick(position: Int) {
                activity?.let { PrefSaveQuestion11(it).setData(data.get(position).id.toString()) }
                openFragment(Question12Fragment())
