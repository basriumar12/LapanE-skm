package com.example.lapane_skm.ui.question.fragment_10
 ...
 *
 */
class Question10Fragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        isDataQuestion = activity?.let { PrefSaveQuestion10(it).getData().toString() }.toString()
        Log.e("data ", "data question 10 $isDataQuestion")
    ...
    private fun initData() {
        tvNo?.text ="10"
        tvQuestion?.text ="Kesesuaian produk pelayanan yang tercantum dalam standar pelayanan dengan hasil yang diberikan? "
        ...
        val adapter = Jabawan10Adapter(activity, data)
        rvDataQuestion?.adapter = adapter
        adapter.notifyDataSetChanged()

        adapter.setOnItemClickListener(object : Jabawan10Adapter.OnItemClickListener {
            override fun onItemClick(position: Int) {
                activity?.let { PrefSaveQuestion10(it).setData(data.get(position).id.toString()) }
                openFragment(Question11Fragment())
